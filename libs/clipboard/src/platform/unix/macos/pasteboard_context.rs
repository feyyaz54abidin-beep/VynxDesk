...
impl Drop for PasteboardContext {
    fn drop(&mut self) {
        self.observer.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).stop();
...
impl CliprdrServiceContext for PasteboardContext {
    ...
    fn get_progress_percent(&self) -> Option<ProgressPercent> {
        self.paste_task.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).progress_percent()
    }

    fn cancel(&mut self) {
        self.paste_task.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).cancel();
    }
}
...
impl PasteboardContext {
        ...
        thread::spawn(move || loop {
            match rx.recv() {
                Ok(Ok(task_info)) => {
                    ...
                    observer.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).start(task_info);
                }
                Ok(Err(e)) => {
    ...
    fn server_clip_file_(&mut self, conn_id: i32, msg: ClipboardFile) -> Result<(), CliprdrError> {
        match msg {
            ClipboardFile::FormatList { format_list } => {
                ...
                let task_lock = self.paste_task.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn handle_format_data_response(
        ...
        let mut task_lock = self.paste_task.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
