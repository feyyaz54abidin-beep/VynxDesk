...
impl Drop for PasteTask {
    fn drop(&mut self) {
        *self.exit.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
...
impl PasteTask {
    ...
    pub fn start(&mut self, target_dir: PathBuf, files: Vec<FileDescription>) {
        let mut task_lock = self.handle.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if task_lock
            .as_ref()
            .map(|x| !x.is_finished())
            .unwrap_or(false)
    ...
    pub fn cancel(&self) {
        let mut task_handle = self.handle.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        ...
        thread::spawn(move || {
            ...
            loop {
                ...
                match rx_file_contents.recv_timeout(Duration::from_millis(300)) {
                    Ok(file_contents) => {
                        let mut task_lock = handle.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
