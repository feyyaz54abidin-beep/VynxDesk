...
impl PasteObserver {
                ...
                Ok(event) => {
                        ...
                        && (event.flag & StreamFlags::IS_FILE) != StreamFlags::NONE
                    {
                        ...
                        if let Some(source_file) = source_file {
                            ...
                            if source_file == file {
                                if let Some(observer_info) = observer_info.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_mut()
                                {
    ...
    pub fn start(&mut self, observer_info: PasteObserverInfo) {
        if let Some(tx_handle_fsevent_thread) = self.tx_handle_fsevent_thread.as_ref() {
            self.observer_info.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).replace(observer_info);
