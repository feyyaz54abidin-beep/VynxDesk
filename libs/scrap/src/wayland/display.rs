...
#[cfg(feature = "drm")]
fn backed_off() -> bool {
    let failed_at = *LAST_FAILED_LOOKUP.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn enumerate_displays() -> hbb_common::ResultType<Vec<WaylandDisplayInfo>> {
    ...
    #[cfg(feature = "drm")]
    {
        *LAST_FAILED_LOOKUP.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = (probed.is_err() && !named).then(Instant::now);
...
pub fn wayland_lookup_suppressed() -> bool {
    DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_none() && backed_off()
}
...
pub fn wayland_failure_stamped() -> bool {
    LAST_FAILED_LOOKUP.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_some()
}

pub fn get_displays() -> Arc<Displays> {
    let mut lock = DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    match lock.as_ref() {
...
pub fn clear_wayland_displays_cache() {
    let _ = DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take();
...
pub fn wayland_snapshot_missing() -> bool {
    DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_none()
}
...
mod tests {
    ...
    fn test_clear_keeps_the_failure_stamp() {
        ...
        *LAST_FAILED_LOOKUP.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(Instant::now());
        clear_wayland_displays_cache();
        let stamp = *LAST_FAILED_LOOKUP.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        assert!(stamp.is_some());
        *LAST_FAILED_LOOKUP.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = None;
    }
