...
pub fn close_session() {
    let _ = RDP_SESSION_INFO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take();
...
pub fn is_rdp_session_hold() -> bool {
    RDP_SESSION_INFO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_some()
}

pub fn try_close_session() {
    let mut rdp_info = RDP_SESSION_INFO.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    let mut close = false;
...
impl PipeWireCapturable {
    fn new(
        ...
        *resolution.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(physical_size);
        Self {
            dbus_conn: conn,
            fd,
            path: stream.path,
...
pub fn request_remote_desktop(
    ...
    for _ in 0..1800 {
        ...
        if fd_res.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_some() {
            break;
        }
    ...
    let fd_res = fd_res.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    let streams_res = streams_res.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    let session_res = session_res.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn fill_displays(
    ...
    let mut rdp_connection = RDP_SESSION_INFO.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    let rdp_info = match rdp_connection.as_mut() {
        Some(res) => res,
        None => {
            // Unreachable
