...
pub extern "system" fn Java_ffi_FFI_onVideoFrameUpdate(
    ...
    if let Ok(data) = env.get_direct_buffer_address(&jb) {
        if let Ok(len) = env.get_direct_buffer_capacity(&jb) {
            VIDEO_RAW.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).update(data, len);
        }
    }
}
...
pub extern "system" fn Java_ffi_FFI_onAudioFrameUpdate(
    ...
    if let Ok(data) = env.get_direct_buffer_address(&jb) {
        if let Ok(len) = env.get_direct_buffer_capacity(&jb) {
            AUDIO_RAW.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).update(data, len);
        }
    }
}
    ...
    if let Ok(data) = env.get_direct_buffer_address(&buffer) {
        if let Ok(len) = env.get_direct_buffer_capacity(&buffer) {
            ...
            if let Ok(clips) = MultiClipboards::parse_from_bytes(&data[1..]) {
                ...
                if is_client {
                    *CLIPBOARDS_CLIENT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(clips);
                } else {
                    *CLIPBOARDS_HOST.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(clips);
                }
            }
        }
    }
...
pub extern "system" fn Java_ffi_FFI_setFrameRawEnable(
    ...
    if let Ok(name) = env.get_string(&name) {
        ...
        if name.eq("video") {
            VIDEO_RAW.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).set_enable(value);
        } else if name.eq("audio") {
            AUDIO_RAW.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).set_enable(value);
        }
    };
}
...
fn init_ndk_context(java_vm: *mut c_void, context_jobject: *mut c_void) {
    let mut lock = NDK_CONTEXT_INITED.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    if *lock {
        unsafe {
