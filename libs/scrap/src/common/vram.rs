...
impl VRamEncoder {
    ...
    pub fn available(format: CodecFormat) -> Vec<FeatureContext> {
        let fallbacks = FALLBACK_GDI_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
        if !fallbacks.is_empty() {
            log::info!("fallback gdi displays not empty: {fallbacks:?}");
            return vec![];
        }
        let not_use = ENOCDE_NOT_USE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
