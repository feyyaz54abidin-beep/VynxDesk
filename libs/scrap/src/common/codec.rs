...
impl Encoder {
        ...
        match config {
            ...
            EncoderCfg::HWRAM(_) => match HwRamEncoder::new(config, i444) {
                ...
                Err(e) => {
                    ...
                    *ENCODE_CODEC_FORMAT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = CodecFormat::VP9;
                    Err(e)
                }
            },
            #[cfg(feature = "vram")]
    ...
    pub fn update(update: EncodingUpdate) {
        ...
        let mut decodings = PEER_DECODINGS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        match update {
        ...
        let mut format = ENCODE_CODEC_FORMAT.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let preferences: Vec<_> = decodings
            ...
            .map(|(_, s)| s.prefer)
            .collect();
        *USABLE_ENCODING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(SupportedEncoding {
            vp8: vp8_useable,
            av1: av1_useable,
            h264: h264_useable,
            h265: h265_useable,
    ...
    pub fn negotiated_codec() -> CodecFormat {
        ENCODE_CODEC_FORMAT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
    }
    ...
    pub fn usable_encoding() -> Option<SupportedEncoding> {
        USABLE_ENCODING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
    }
    ...
    pub fn set_fallback(config: &EncoderCfg) {
        ...
        let current = ENCODE_CODEC_FORMAT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
        if current != format {
            log::info!("codec fallback: {:?} -> {:?}", current, format);
            *ENCODE_CODEC_FORMAT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = format;
        }
    }

    pub fn use_i444(config: &EncoderCfg) -> bool {
        let decodings = PEER_DECODINGS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
...
pub fn codec_thread_num(limit: usize) -> usize {
    ...
    // avoid frequent log
    let log = match THREAD_LOG_TIME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone() {
        Some(instant) => instant.elapsed().as_secs() > 1,
        None => true,
    };
    if log {
        log::info!("cpu num: {max}, {info}, available memory: {memory}G, codec thread: {res}");
        *THREAD_LOG_TIME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(Instant::now());
    }
    res
}
