...
impl HwCodecConfig {
    ...
    pub fn set(config: String) {
        ...
        *CONFIG_SET_BY_IPC.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
    }
    ...
    pub fn get() -> HwCodecConfig {
            ...
            match config {
                ...
                None => {
                    ...
                    if c.signature == new_signature {
                        ...
                        *CONFIG.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(c.clone());
                        c
                    } else {
                        log::info!(
        ...
        #[cfg(target_os = "linux")]
        {
            CONFIG.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone().unwrap_or_default()
        }
        #[cfg(target_os = "ios")]
        {
            HwCodecConfig::default()
    ...
    pub fn get_set_value() -> Option<HwCodecConfig> {
        let set = CONFIG_SET_BY_IPC.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
        if set {
            CONFIG.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
        } else {
            None
        }
    }
    ...
    pub fn already_set() -> bool {
        CONFIG_SET_BY_IPC.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
    }
    ...
    pub fn clear(vram: bool, encode: bool) {
        ...
        #[cfg(not(target_os = "android"))]
        {
            let mut c = CONFIG.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
