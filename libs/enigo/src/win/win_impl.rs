...
const INPUT_PROFILE_OPTION: &str = "windows-input-compatibility-profile";

#[cfg(feature = "windows-user-input-plus")]
static LEGACY_MOUSE_BUTTONS: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);

#[cfg(feature = "windows-user-input-plus")]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum InputCompatibilityProfile {
    Standard,
    ScanCode,
    VirtualKey,
    LegacyWindow,
...
#[cfg(feature = "windows-user-input-plus")]
impl InputCompatibilityProfile {
    fn parse(value: &str) -> Self {
        match value.trim().to_ascii_lowercase().as_str() {
            "scan-code" | "scancode" => Self::ScanCode,
            "virtual-key" | "virtualkey" => Self::VirtualKey,
            "legacy-window" | "window-message" => Self::LegacyWindow,
            _ => Self::Standard,
        }
    }
}
...
#[cfg(feature = "windows-user-input-plus")]
fn input_compatibility_profile() -> InputCompatibilityProfile {
    use std::sync::OnceLock;

    static PROFILE: OnceLock<InputCompatibilityProfile> = OnceLock::new();
    *PROFILE.get_or_init(|| {
        let profile = InputCompatibilityProfile::parse(&hbb_common::config::Config::get_option(
            INPUT_PROFILE_OPTION,
        ));
        log::info!("Windows input compatibility profile: {:?}", profile);
        profile
    })
}
...
#[cfg(feature = "windows-user-input-plus")]
fn post_legacy_keyboard_event(flags: u32, vk: u16, scan: u16) -> DWORD {
    unsafe {
        ...
        if flags & KEYEVENTF_UNICODE != 0 {
            ...
            return PostMessageW(hwnd, WM_CHAR, scan as WPARAM, 1) as DWORD;
        }
        let msg = if flags & KEYEVENTF_KEYUP != 0 {
            WM_KEYUP
        } else {
            WM_KEYDOWN
        };
        PostMessageW(hwnd, msg, vk as WPARAM, legacy_keyboard_lparam(scan, flags)) as DWORD
    }
}

#[cfg(feature = "windows-user-input-plus")]
fn legacy_mouse_key_state() -> WPARAM {
    use std::sync::atomic::Ordering;

    let mut state = LEGACY_MOUSE_BUTTONS.load(Ordering::Relaxed);
    unsafe {
...
#[cfg(feature = "windows-user-input-plus")]
fn update_legacy_mouse_buttons(flags: u32, data: u32) -> (usize, usize) {
    use std::sync::atomic::Ordering;

    let (down, up, mask) = if flags & (MOUSEEVENTF_LEFTDOWN | MOUSEEVENTF_LEFTUP) != 0 {
        (
        ...
        let mask = if data == XBUTTON1 as u32 {
            MK_XBUTTON1
        } else {
            MK_XBUTTON2
        };
        (MOUSEEVENTF_XDOWN, MOUSEEVENTF_XUP, mask)
    } else {
        let current = LEGACY_MOUSE_BUTTONS.load(Ordering::Relaxed);
        return (current, current);
    };

    if flags & down != 0 {
        let previous = LEGACY_MOUSE_BUTTONS.fetch_or(mask, Ordering::Relaxed);
        (previous, previous | mask)
    } else if flags & up != 0 {
        let previous = LEGACY_MOUSE_BUTTONS.fetch_and(!mask, Ordering::Relaxed);
        (previous, previous & !mask)
    } else {
        let current = LEGACY_MOUSE_BUTTONS.load(Ordering::Relaxed);
        (current, current)
...
#[cfg(feature = "windows-user-input-plus")]
fn post_legacy_mouse_event(flags: u32, data: u32) -> Option<DWORD> {
    use std::sync::atomic::Ordering;

    let supported = MOUSEEVENTF_LEFTDOWN
        | MOUSEEVENTF_LEFTUP
        | MOUSEEVENTF_RIGHTDOWN
        | MOUSEEVENTF_RIGHTUP
        | MOUSEEVENTF_MIDDLEDOWN
        ...
        | MOUSEEVENTF_WHEEL
        | MOUSEEVENTF_HWHEEL;
    if input_compatibility_profile() != InputCompatibilityProfile::LegacyWindow
        || flags & supported == 0
    {
        return None;
    }
    ...
    unsafe {
        ...
        let is_wheel = flags & (MOUSEEVENTF_WHEEL | MOUSEEVENTF_HWHEEL) != 0;
        let mut message_point = point;
        if !is_wheel && ScreenToClient(hwnd, &mut message_point) == 0 {
            return Some(0);
        }
        let lparam = ((message_point.y as u16 as u32) << 16) | message_point.x as u16 as u32;
        let (previous_buttons, current_buttons) = update_legacy_mouse_buttons(flags, data);
        let (message, mut wparam) = if flags & MOUSEEVENTF_LEFTDOWN != 0 {
            (WM_LBUTTONDOWN, current_buttons)
        } else if flags & MOUSEEVENTF_LEFTUP != 0 {
            (WM_LBUTTONUP, current_buttons)
        } else if flags & MOUSEEVENTF_RIGHTDOWN != 0 {
            (WM_RBUTTONDOWN, current_buttons)
        } else if flags & MOUSEEVENTF_RIGHTUP != 0 {
            (WM_RBUTTONUP, current_buttons)
        } else if flags & MOUSEEVENTF_MIDDLEDOWN != 0 {
            (WM_MBUTTONDOWN, current_buttons)
        } else if flags & MOUSEEVENTF_MIDDLEUP != 0 {
            (WM_MBUTTONUP, current_buttons)
        } else if flags & MOUSEEVENTF_XDOWN != 0 {
            (WM_XBUTTONDOWN, current_buttons)
        } else if flags & MOUSEEVENTF_XUP != 0 {
            (WM_XBUTTONUP, current_buttons)
        } else if flags & MOUSEEVENTF_HWHEEL != 0 {
            (WM_MOUSEHWHEEL, legacy_mouse_key_state())
        } else {
            (WM_MOUSEWHEEL, legacy_mouse_key_state())
        };
        if flags & (MOUSEEVENTF_XDOWN | MOUSEEVENTF_XUP) != 0 || is_wheel {
            wparam |= (data as usize & 0xFFFF) << 16;
        }

        let result = PostMessageW(hwnd, message, wparam, lparam as LPARAM) as DWORD;
        if result == 0 {
            LEGACY_MOUSE_BUTTONS.store(previous_buttons, Ordering::Relaxed);
        }
        Some(result)
    }
}
...
fn mouse_event(flags: u32, data: u32, dx: i32, dy: i32) -> DWORD {
    ...
    if super::bridge::mouse_event(flags, data, dx, dy) {
        return 1;
    }

    #[cfg(feature = "windows-user-input-plus")]
    if let Some(result) = post_legacy_mouse_event(flags, data) {
        return result;
    }
    ...
    let res = unsafe { SendInput(1, &mut input as LPINPUT, size_of::<INPUT>() as c_int) };
    if res == 0
        && (flags
            & (MOUSEEVENTF_LEFTDOWN
                | MOUSEEVENTF_LEFTUP
        ...
        unsafe {
            ...
            if GetCursorPos(&mut pt) != 0 {
                ...
                if !hwnd.is_null() {
                    ...
                    let lparam = ((client_pt.y as u16 as u32) << 16) | (client_pt.x as u16 as u32);
                    if flags & MOUSEEVENTF_LEFTDOWN != 0 {
                        PostMessageW(hwnd, WM_LBUTTONDOWN, MK_LBUTTON as WPARAM, lparam as LPARAM);
                    }
                    if flags & MOUSEEVENTF_LEFTUP != 0 {
                        PostMessageW(hwnd, WM_LBUTTONUP, 0, lparam as LPARAM);
                    }
                    if flags & MOUSEEVENTF_RIGHTDOWN != 0 {
                        PostMessageW(hwnd, WM_RBUTTONDOWN, MK_RBUTTON as WPARAM, lparam as LPARAM);
                    }
                    if flags & MOUSEEVENTF_RIGHTUP != 0 {
                        PostMessageW(hwnd, WM_RBUTTONUP, 0, lparam as LPARAM);
                    }
                    if flags & MOUSEEVENTF_MIDDLEDOWN != 0 {
                        PostMessageW(hwnd, WM_MBUTTONDOWN, MK_MBUTTON as WPARAM, lparam as LPARAM);
                    }
                    if flags & MOUSEEVENTF_MIDDLEUP != 0 {
                        PostMessageW(hwnd, WM_MBUTTONUP, 0, lparam as LPARAM);
                    }
                }
            }
        }
...
fn keybd_event(mut flags: u32, vk: u16, scan: u16) -> DWORD {
    let mut scan = scan;
    #[cfg(feature = "windows-user-input-plus")]
    let profile = input_compatibility_profile();
    unsafe {
        // https://github.com/rustdesk/rustdesk/issues/366
        if scan == 0 {
            let hwnd = GetForegroundWindow();
            let current_window_thread_id = if !hwnd.is_null() {
                GetWindowThreadProcessId(hwnd, std::ptr::null_mut())
            } else {
                0
            };
            let mut layout = if current_window_thread_id != 0 {
            ...
            scan = MapVirtualKeyExW(vk as _, 4, LAYOUT) as _;
            if scan == 0 {
    ...
    #[cfg(feature = "windows-user-input-plus")]
    if flags & KEYEVENTF_UNICODE == 0 && profile == InputCompatibilityProfile::VirtualKey {
        scan = 0;
    ...
    if flags & KEYEVENTF_UNICODE == 0 {
        ...
        if scan != 0 {
            flags |= winapi::um::winuser::KEYEVENTF_SCANCODE;
        }
    }

    #[cfg(feature = "vynx-input-bridge")]
    if super::bridge::key_event(vk, scan, flags) {
        return 1;
    }

    #[cfg(feature = "windows-user-input-plus")]
    if profile == InputCompatibilityProfile::LegacyWindow {
        return post_legacy_keyboard_event(flags, vk, scan);
    }
    ...
    if res == 0 && (flags & KEYEVENTF_UNICODE == 0) {
        unsafe {
            let hwnd = GetForegroundWindow();
            if !hwnd.is_null() && vk != 0 {
                let msg = if flags & KEYEVENTF_KEYUP != 0 {
                    WM_KEYUP
                } else {
                    WM_KEYDOWN
...
impl MouseControllable for Enigo {
    ...
    fn mouse_move_to(&mut self, x: i32, y: i32) {
        #[cfg(feature = "vynx-input-bridge")]
        if super::bridge::absolute_mouse(
...
#[cfg(all(test, feature = "windows-user-input-plus"))]
mod input_compatibility_tests {
    use super::*;

    #[test]
    fn profile_aliases_are_stable() {
        assert_eq!(
            InputCompatibilityProfile::parse("scan-code"),
            InputCompatibilityProfile::ScanCode
        );
        assert_eq!(
            InputCompatibilityProfile::parse("VIRTUALKEY"),
            InputCompatibilityProfile::VirtualKey
        );
        assert_eq!(
            InputCompatibilityProfile::parse("window-message"),
            InputCompatibilityProfile::LegacyWindow
        );
        assert_eq!(
            InputCompatibilityProfile::parse("unknown"),
            InputCompatibilityProfile::Standard
        );
    }

    #[test]
    fn legacy_keyup_lparam_contains_transition_state() {
        let value = legacy_keyboard_lparam(0x1D, KEYEVENTF_KEYUP | KEYEVENTF_EXTENDEDKEY);
        assert_eq!(value & 0xFF, 1);
        assert_eq!((value >> 16) & 0xFF, 0x1D);
        assert_ne!(value & (1 << 24), 0);
        assert_ne!(value & (1 << 30), 0);
        assert_ne!(value & (1 << 31), 0);
    }

    #[test]
    fn legacy_mouse_button_state_tracks_down_and_up() {
        use std::sync::atomic::Ordering;

        LEGACY_MOUSE_BUTTONS.store(0, Ordering::Relaxed);
        let (previous, current) = update_legacy_mouse_buttons(MOUSEEVENTF_LEFTDOWN, 0);
        assert_eq!(previous, 0);
        assert_eq!(current, MK_LBUTTON as usize);
