...
pub fn is_device_created() -> bool {
    #[cfg(windows)]
    return *H_SW_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) != 0;
    #[cfg(not(windows))]
    return false;
}
...
pub fn create_device() -> ResultType<()> {
    ...
    #[cfg(windows)]
    unsafe {
        let mut lock_device = H_SW_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn close_device() {
    #[cfg(windows)]
    unsafe {
        win10::idd::DeviceClose(*H_SW_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) as win10::idd::HSWDEVICE);
        *H_SW_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = 0;
    }
}
