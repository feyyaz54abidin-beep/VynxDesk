...
pub fn get_driver_install_path() -> Option<&'static str> {
    Some(LIB_WRAPPER.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_driver_install_path?())
}
...
pub fn close_device() {
    let _r = LIB_WRAPPER.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).close_device.map(|f| f());
}
...
pub fn plug_in_monitor(monitor_index: u32) -> ResultType<()> {
    let mut lock = MONITOR_INDICES.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn plug_out_monitor(monitor_index: u32) -> ResultType<()> {
    ...
    f(monitor_index)?;
    MONITOR_INDICES.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&monitor_index);
    Ok(())
}
