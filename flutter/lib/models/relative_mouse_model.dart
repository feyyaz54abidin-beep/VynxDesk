...
class RelativeMouseModel {
  ...
  bool handleKeyEvent(
    ...
    if (enabled.value) {
      ...
      // Toggle shortcut: Ctrl+Alt+Shift+M (or Cmd+Shift+M on macOS)
      if (isDesktop && keyboardPerm() && !isViewCamera() && (e is KeyDownEvent)) {
        if (e.logicalKey == LogicalKeyboardKey.keyM) {
          final isShortcut = isMacOS
              ? (commandPressed && shiftPressed)
              : (ctrlPressed && altPressed && shiftPressed);
          if (isShortcut && isSupported) {
  ...
  bool handleRawKeyEvent(RawKeyEvent e) {
    ...
    if (enabled.value) {
      ...
      if (isDesktop && keyboardPerm() && !isViewCamera() && (e is RawKeyDownEvent)) {
        if (e.logicalKey == LogicalKeyboardKey.keyM) {
          final isShortcut = isMacOS
              ? (modifiers.isMetaPressed && modifiers.isShiftPressed)
              : (modifiers.isControlPressed &&
