...
impl SendCredit {
    ...
    pub async fn take(&self, max: usize) -> usize {
        ...
        loop {
            {
                let mut credit = self.credit.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
async fn relay_tunnel_to_socket<W: AsyncWrite + Unpin>(
    ...
    loop {
        ...
        let update = window.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).drained(chunk.len());
    ...
    impl TunnelHandle {
        ...
        pub fn open(
            ...
            tokio::spawn(async move {
                ...
                handle.channels.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&id);
            });
            Ok(())
        }
        ...
        pub(super) fn on_frame(&self, ch: PortForwardChannel) -> Option<String> {
            match ch.union {
                ...
                Some(port_forward_channel::Union::Data(d)) => {
                    ...
                    let accepted = e.window.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).accept(d.data.len());
                ...
                Some(port_forward_channel::Union::Close(c)) => {
                    if let Some(e) = self.channels.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&c.channel_id) {
                        e.inbound.send(Inbound::Close).ok();
                    }
                    None
                }
                Some(port_forward_channel::Union::WindowUpdate(u)) => {
                    if let Some(e) = self.channels.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get(&u.channel_id) {
                        e.credit.add(u.add);
                    }
                    None
                }
        ...
        fn close_all(&self) {
            self.channels.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clear();
        ...
        pub fn live_channels(&self) -> usize {
            self.channels.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).len()
        }
...
mod tests {
    ...
    fn inbound_data_is_written_and_window_update_follows_threshold() {
        rt().block_on(async {
            ...
            assert!(h.window.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).accept(chunk.len()));
    ...
    mod tunnel {
        ...
        impl crate::client::Interface for NoUi {
            ...
            fn msgbox(&self, _msgtype: &str, _title: &str, text: &str, _link: &str) {
                self.0.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(text.to_owned());
            }
