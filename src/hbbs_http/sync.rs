...
pub fn signal_receiver() -> broadcast::Receiver<Vec<i32>> {
    SENDER.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).subscribe()
}
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                ...
                if url.is_empty() {
                    *PRO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = false;
                    continue;
                }
                    ...
                    if crate::is_public(&url) {
                        ...
                        if hash == old_hash {
                            ...
                            let samever = match crate::post_request(url.replace("heartbeat", "sysinfo_ver"), "".to_owned(), "").await {
                                Ok(x)  => {
                                    sysinfo_ver = x.clone();
                                    *PRO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
                                    x == ver
                                }
                                _ => {
                    ...
                    if let Ok(mut rsp) = serde_json::from_str::<HashMap::<&str, Value>>(&s) {
                        ...
                        if let Some(conns)  = rsp.remove("disconnect") {
                                if let Ok(conns) = serde_json::from_value::<Vec<i32>>(conns) {
                                    SENDER.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).send(conns).ok();
                                }
                        }
...
#[cfg(not(any(target_os = "ios")))]
pub fn is_pro() -> bool {
    PRO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
}
