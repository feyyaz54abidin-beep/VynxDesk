...
pub fn download_file(
    ...
    {
        let mut downloaders = DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    std::thread::spawn(
        move || match do_download(&id2, url, path, auto_del_dur, rx) {
            Ok(is_all_downloaded) => {
                ...
                DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_mut(&id2).map(|downloader| {
                ...
                if is_canceled {
                    if let Some(downloader) = DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&id2) {
...
async fn do_download(
            ...
            match head_resp {
                Ok(resp) => {
                    if resp.status().is_success() {
                        ...
                        DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_mut(id).map(|downloader| {
    ...
    if let Some(ref mut downloader) = DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_mut(id) {
    ...
    if is_all_downloaded {
        ...
        if let Some(dur) = auto_del_dur {
            tokio::spawn(async move {
                ...
                DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&id_del);
            });
        }
    }
    Ok(is_all_downloaded)
...
pub fn get_download_data(id: &str) -> ResultType<DownloadData> {
    let downloaders = DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    if let Some(downloader) = downloaders.get(id) {
...
pub fn cancel(id: &str) {
    if let Some(downloader) = DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get(id) {
...
pub fn remove(id: &str) {
    let _ = DOWNLOADERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(id);
}
