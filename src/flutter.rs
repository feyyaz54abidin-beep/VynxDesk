    ...
    pub fn has_connected_sessions_running(conn_type: ConnType) -> bool {
        SESSIONS.read().unwrap().iter().any(|((_, r#type), s)| {
            *r#type == conn_type
                ...
                && s.connection_round_state.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_connected()
        })
    }
}
...
pub(super) mod async_tasks {
    ...
    pub fn stop_flutter_async_runner() {
        let _ = TX_QUERY_ONLINES.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take();
    }
    ...
    async fn start_flutter_async_runner_() {
        ...
        TX_QUERY_ONLINES.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).replace(tx_onlines);
    ...
    pub fn query_onlines(ids: Vec<String>) -> ResultType<()> {
        if let Some(tx) = TX_QUERY_ONLINES.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_ref() {
