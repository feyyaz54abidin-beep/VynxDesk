...
pub fn start_auto_update() {
    let _sender = TX_MSG.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
}

#[allow(dead_code)]
pub fn manually_check_update() -> ResultType<()> {
    let sender = TX_MSG.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn check_update(manually: bool) -> ResultType<()> {
    ...
    let update_url = crate::common::SOFTWARE_UPDATE_URL.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
