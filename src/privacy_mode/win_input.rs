...
fn do_hook(tx: Sender<String>) -> ResultType<(HHOOK, HHOOK)> {
    ...
    let mut cur_hook_thread_id = CUR_HOOK_THREAD_ID.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn hook() -> ResultType<()> {
    ...
    std::thread::spawn(move || {
        ...
        unsafe {
            ...
            *CUR_HOOK_THREAD_ID.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = 0;
        }
    });
...
pub fn unhook() -> ResultType<()> {
    unsafe {
        let cur_hook_thread_id = CUR_HOOK_THREAD_ID.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if *cur_hook_thread_id != 0 {
