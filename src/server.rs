...
pub async fn create_tcp_connection(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if let Ok(task) = Command::new("/usr/bin/caffeinate")
            ...
            .spawn()
        {
            super::CHILD_PROCESS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(task);
        }
...
impl Server {
    ...
    fn update_enable_retina(&self) {
        ...
        *scrap::quartz::ENABLE_RETINA.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = video_service_count < 2;
    }
}
...
pub fn check_zombie() {
    std::thread::spawn(|| loop {
        let mut lock = CHILD_PROCESS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let mut i = 0;
        while i != lock.len() {
            let c = &mut (*lock)[i];
