    ...
    if let Ok(s) = ipc::connect(1000, "_whiteboard").await {
        ...
        if crate::platform::is_root() {
            ...
            if let Some(task) = res? {
                CHILD_PROCESS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(task);
            }
            run_done = true;
        } else {
            run_done = false;
        ...
        if !run_done {
            log::debug!("Start whiteboard");
            CHILD_PROCESS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(crate::run_me(args)?);
        }
        for _ in 0..20 {
            sleep(0.3).await;
            if let Ok(s) = ipc::connect(1000, "_whiteboard").await {
