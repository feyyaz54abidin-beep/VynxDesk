...
pub mod client {
    ...
    fn apply_run_grab_if_owner(session_id: u128, disable_first: bool) {
        let _lock = GRAB_OP_LOCK.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let gs = GRAB_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn start_grab_loop() {
        let mut lock = IS_GRAB_STARTED.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if *lock {
            return;
        }
    ...
    pub fn change_grab_status(state: GrabState, keyboard_mode: &str, session_id: u128) {
        ...
        let mut gs = GRAB_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        match state {
            GrabState::Ready => {}
            GrabState::Run => {
                #[cfg(windows)]
    ...
    pub fn get_modifiers_state(
        ...
        let modifiers_lock = MODIFIERS_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn get_keyboard_mode() -> String {
    #[cfg(not(feature = "flutter"))]
    if let Some(session) = CUR_SESSION.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_ref() {
...
fn is_exit_relative_mouse_shortcut(key: Key) -> bool {
    let modifiers = MODIFIERS_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn is_long_press(event: &Event) -> bool {
    let keys = MODIFIERS_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn take_remote_keys() -> HashMap<Key, Event> {
    let mut to_release = TO_RELEASE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn update_modifiers_state(event: &Event) {
    // for mouse
    let mut keys = MODIFIERS_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    match event.event_type {
        EventType::KeyPress(k) => {
...
pub fn event_to_key_events(
    ...
    match event.event_type {
        EventType::KeyPress(key) => {
            TO_RELEASE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).insert(key, event.clone());
        }
        EventType::KeyRelease(key) => {
            TO_RELEASE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&key);
        }
        _ => {}
    }
