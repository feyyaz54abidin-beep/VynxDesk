...
pub fn installing_service() -> bool {
    INSTALLING_SERVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
}
...
impl InstallingService {
    pub fn new() -> Self {
        *INSTALLING_SERVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
        Self
    }
}
...
impl Drop for InstallingService {
    fn drop(&mut self) {
        *INSTALLING_SERVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = false;
    }
}
