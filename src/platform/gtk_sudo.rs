...
fn ui(args: Vec<String>) {
    ...
    application.connect_activate(glib::clone!(@weak application =>move |_| {
        ...
        glib::timeout_add_local(std::time::Duration::from_millis(50), move || {
            if let Ok(msg) = rx_to_ui.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).try_recv() {
                match msg {
                    Message::PasswordPrompt((err_msg, show_edit)) => {
                        let last_pwd = last_password.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
                        let username = username.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
                        if let Some((username, password)) = password_prompt(&username, &last_pwd, &err_msg, show_edit) {
                                *last_password.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = password.clone();
                            ...
                            if let Err(e) = tx_from_ui.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).send(Message::Cancel) {
                                error_dialog_and_exit(&format!("Channel error: {e:?}"), EXIT_CODE);
                            }
                        }
                    }
        ...
        if acitve_user != "root" {
            ...
            match rx_from_ui.recv() {
                Ok(Message::Password((user, password))) => {
                    *username.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = user;
