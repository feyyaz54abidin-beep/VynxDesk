...
pub fn try_change_desktop() -> bool {
    unsafe {
        if inputDesktopSelected() == FALSE {
            ...
            if !res {
                let mut s = SUPPRESS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn set_vynx_caption_colors(hwnd: winapi::shared::windef::HWND) {
    #[link(name = "dwmapi")]
    extern "system" {
    ...
    // COLORREF uses BGR. Unsupported attributes keep the system frame intact.
    for (attribute, value) in [(20u32, 1u32), (34, 0x00463024), (35, 0x00140c06), (36, 0x00fcf4f6)] {
        let result = unsafe {
            DwmSetWindowAttribute(hwnd, attribute, &value as *const u32 as _, 4)
        };
        if result < 0 {
            log::debug!("Vynx caption attribute {} unavailable: {:#x}", attribute, result);
        }
    }
}
