...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn send_note(&self, note: String) {
        ...
        *self.last_audit_note.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = note.clone();
    ...
    pub fn reconnect(&self, force_relay: bool) {
        ...
        let mut connection_round_state_lock = self.connection_round_state.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if self.thread.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_some() {
        ...
        let mut lock = self.thread.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn set_custom_resolution(&self, display: &SwitchDisplay) {
            ...
            && display.height == display.original_resolution.height
        {
            ...
            let last_change_display = self.last_change_display.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn change_resolution(&self, display: i32, width: i32, height: i32) {
        *self.last_change_display.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) =
...
impl<T: InvokeUiSession> Interface for Session<T> {
    ...
    fn handle_peer_info(&self, mut pi: PeerInfo) {
        ...
        *self.audit_guid.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = String::new();
        *self.last_audit_note.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = String::new();
