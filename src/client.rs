...
pub fn get_key_state(key: enigo::Key) -> bool {
    ...
    ENIGO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_key_state(key)
}
...
impl Client {
    ...
    async fn _start_inner(
        ...
        if !key.is_empty() && (!token.is_empty() || !switch_code.is_empty()) {
            ...
            loop {
                let port = *udp.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                if port > 0 {
                    break;
                }
        ...
        let mut ipv6 = ipv6
            ...
            .unwrap_or((None, None));
        let udp_nat_port = udp.1.map(|x| *x.lock().unwrap_or_else(|poisoned| poisoned.into_inner())).unwrap_or(0);
    ...
    pub fn set_is_text_clipboard_required(b: bool) {
        CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_text_required = b;
    }

    #[inline]
    #[cfg(all(feature = "flutter", feature = "unix-file-copy-paste"))]
    pub fn set_is_file_clipboard_required(b: bool) {
        CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_file_required = b;
    }
    ...
    fn try_stop_clipboard() {
        ...
        CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).running = false;
    ...
    fn try_start_clipboard(
        ...
        let mut clipboard_lock = CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        ...
        std::thread::spawn(move || {
            ...
            tx_started.send(()).ok();
            loop {
                if !CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).running {
                    break;
                }
    ...
    fn try_start_clipboard(_p: Option<()>) -> Option<UnboundedReceiver<()>> {
        ...
        std::thread::spawn(move || {
            loop {
                ...
                if !CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_text_required {
...
impl ClientClipboardHandler {
    fn is_text_required(&self) -> bool {
        #[cfg(feature = "flutter")]
        {
            CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_text_required
        }
        #[cfg(not(feature = "flutter"))]
        {
    ...
    fn is_file_required(&self) -> bool {
        #[cfg(feature = "flutter")]
        {
            CLIPBOARD_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_file_required
        }
        #[cfg(not(feature = "flutter"))]
        {
            self.client_clip_ctx
...
impl AudioHandler {
    ...
    pub fn handle_frame(&mut self, frame: AudioFrame) {
        #[cfg(not(target_os = "linux"))]
        if self.audio_stream.is_none() || !self.ready.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone() {
            return;
        }
        #[cfg(target_os = "linux")]
    ...
    fn build_output_stream<T: cpal::Sample + cpal::SizedSample + cpal::FromSample<f32>>(
        ...
        let stream = device.build_output_stream(
            ...
            move |data: &mut [T], info: &cpal::OutputCallbackInfo| {
                if !*ready.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) {
                    *ready.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
                }

                let mut n = data.len();
                let mut lock = audio_buffer.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                ...
                if having < n {
                    ...
                    if how_long > Duration::from_millis(6) && how_long < Duration::from_millis(3000)
                    {
                        ...
                        lock = audio_buffer.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn handle_frame(
        ...
        match &vf.union {
            Some(frame) => {
                ...
                if self.record {
                    self.recorder.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_mut().map(|r| {
...
impl LoginConfigHandler {
    ...
    fn get_option_message(&self, ignore_default: bool) -> Option<OptionMessage> {
        ...
        if let Some(q) = self.get_image_quality_enum(&q, ignore_default) {
            ...
            if let Some(custom_fps) = self.options.get("custom-fps") {
                ...
                *self.custom_fps.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(custom_fps as _);
            }
        }
    ...
    pub fn set_custom_fps(&mut self, fps: i32, save_config: bool) -> Message {
        ...
        *self.custom_fps.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(fps as _);
        msg_out
    }
...
async fn test_udp_uat(
                        ...
                        match RendezvousMessage::parse_from_bytes(&buf[0..n]) {
                            Ok(msg_in) => {
                                if let Some(rendezvous_message::Union::TestNatResponse(response)) = msg_in.union {
                                    *udp_port.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = response.port as u16;
                                    break;
                                }
                            }
                            Err(e) => {
    ...
    let final_port = *udp_port.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    log::debug!(
