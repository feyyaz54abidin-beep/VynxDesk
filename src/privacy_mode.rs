...
lazy_static::lazy_static! {
    ...
    static ref PRIVACY_MODE: Arc<Mutex<Option<Box<dyn PrivacyMode>>>> = {
        ...
        let privacy_mode = match PRIVACY_MODE_CREATOR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get(&(&cur_impl as &str)) {
...
pub fn init() -> Option<ResultType<()>> {
    Some(PRIVACY_MODE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_ref()?.init())
}

#[inline]
pub fn clear() -> Option<()> {
    Some(PRIVACY_MODE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_mut()?.clear())
}

#[inline]
pub fn switch(impl_key: &str) {
    let mut privacy_mode_lock = PRIVACY_MODE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    if let Some(creator) = PRIVACY_MODE_CREATOR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get(impl_key) {
