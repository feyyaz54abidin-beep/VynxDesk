...
pub fn set_screenshot(data: bytes::Bytes) {
    SCREENSHOT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).set_screenshot(data);
}

pub fn handle_screenshot(action: String) -> String {
    SCREENSHOT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).handle_screenshot(action)
}
