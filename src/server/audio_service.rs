...
pub fn get_voice_call_input_device() -> Option<String> {
    VOICE_CALL_INPUT_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
}

#[inline]
pub fn set_voice_call_input_device(device: Option<String>, set_if_present: bool) {
    if !set_if_present && VOICE_CALL_INPUT_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_some() {
        return;
    }

    if *VOICE_CALL_INPUT_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) == device {
        return;
    }
    *VOICE_CALL_INPUT_DEVICE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = device;
    restart();
}
...
mod cpal_impl {
        ...
        T: cpal::SizedSample + dasp::sample::ToSample<f32>,
    {
        ...
        INPUT_BUFFER.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clear();
        let timeout = None;
        ...
        let stream = device.build_input_stream(
            ...
            move |data: &[T], _: &InputCallbackInfo| {
                ...
                let mut lock = INPUT_BUFFER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                lock.extend(buffer);
