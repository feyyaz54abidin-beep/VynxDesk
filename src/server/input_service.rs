...
fn update_last_cursor_pos(x: i32, y: i32) {
    let mut lock = LATEST_SYS_CURSOR_POS
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn run_pos(sp: EmptyExtraFieldService, state: &mut StatePos) -> ResultType<()> {
    let (_, (x, y)) = *LATEST_SYS_CURSOR_POS
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    if state.is_moved(x, y) {
        ...
        let exclude = {
            let now = get_time();
            let lock = LATEST_PEER_INPUT_CURSOR
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn set_relative_mouse_active(conn: i32, active: bool) {
    let mut lock = RELATIVE_MOUSE_CONNS
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    if active {
        lock.insert(conn);
...
pub fn update_latest_input_cursor_time(conn: i32) {
    let mut lock = LATEST_PEER_INPUT_CURSOR
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    lock.conn = conn;
    lock.time = get_time();
...
fn active_mouse_(_conn: i32) -> bool {
    ...
    if LATEST_PEER_INPUT_CURSOR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).conn != conn {
        return false;
    }
    ...
    match crate::get_cursor_pos() {
        Some((x, y)) => {
            ...
            if !can_active {
                let mut lock = LATEST_PEER_INPUT_CURSOR.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn simulate_(event_type: &EventType) {
    ...
    if enigo::bridge_rdev_event(event_type) {
        return;
    }
...
fn record_wayland_clipboard_input_for_sync_filter(text: &str) -> Option<(Instant, String)> {
    ...
    let now = Instant::now();
    let mut records = WAYLAND_CLIPBOARD_INPUT_RECORDS
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
