...
pub fn notify_video_frame_fetched(display_idx: usize, conn_id: i32, frame_tm: Option<Instant>) {
    if let Some(notifier) = FRAME_FETCHED_NOTIFIERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get(&display_idx) {
...
pub fn notify_video_frame_fetched_by_conn_id(conn_id: i32, frame_tm: Option<Instant>) {
    let vec_display_idx: Vec<usize> = {
        let display_conn_ids = DISPLAY_CONN_IDS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        display_conn_ids
            .iter()
    ...
    let notifiers = FRAME_FETCHED_NOTIFIERS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn run(vs: VideoService) -> ResultType<()> {
    ...
    let mut video_qos = VIDEO_QOS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    VIDEO_QOS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).store_bitrate(encoder.bitrate());
    VIDEO_QOS
        .lock()
        .unwrap()
    ...
    while sp.ok() {
        ...
        let res = match c.frame(spf) {
            Ok(frame) => {
                ...
                if frame.valid() {
                    ...
                    let screenshot = SCREENSHOTS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&screenshot_key);
        ...
        DISPLAY_CONN_IDS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&display_idx);
...
impl Raii {
    fn new(display_idx: usize, name: String) -> Self {
        ...
        VIDEO_QOS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).new_display(name.clone());
        Raii {
            display_idx,
            name,
            try_vram: true,
...
impl Drop for Raii {
    fn drop(&mut self) {
        ...
        VIDEO_QOS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove_display(&self.name);
        DISPLAY_CONN_IDS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remove(&self.display_idx);
    }
}
        ...
        if !crate::platform::is_installed() && !crate::platform::is_root() {
            std::thread::spawn(|| loop {
                ...
                if let Ok(uac) = is_process_consent_running() {
                    *IS_UAC_RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = uac;
                }
                if !crate::platform::is_elevated(None).unwrap_or(false) {
                    if let Ok(elevated) = crate::platform::is_foreground_window_elevated() {
                        *IS_FOREGROUND_WINDOW_ELEVATED.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = elevated;
                    }
                }
            });
        }
...
pub fn set_take_screenshot(source: VideoSource, display_idx: usize, sid: String, tx: Sender) {
    SCREENSHOTS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).insert(
        (source, display_idx),
        Screenshot {
            sid,
            tx,
