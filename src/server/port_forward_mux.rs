...
impl PortForwardMux {
    ...
    pub fn handle(&mut self, frame: PortForwardChannel, tunnel_permitted: impl FnOnce() -> bool) {
        match frame.union {
            ...
            Some(port_forward_channel::Union::Data(d)) => {
                ...
                let accepted = entry.window.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).accept(len);
    ...
    pub fn recv_window_remaining(&self, id: i32) -> Option<u32> {
        self.channels.get(&id).map(|e| e.window.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).remaining())
    }
...
async fn run_controlled_channel(
    ...
    window.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).grant(CHANNEL_WINDOW - INITIAL_WINDOW);
    if sink
