...
pub mod service {
    ...
    fn input_text_wayland(text: &str, keyboard: &mut VirtualDevice) {
        let portal_info = {
            let session_info = RDP_SESSION_INFO.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            session_info
                .as_ref()
    ...
    fn spawn_mouse_handler(mut stream: ipc::Connection) {
        let resolution = RESOLUTION.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                                ...
                                match data {
                                    Data::Control(data) => match data {
                                        ipc::DataControl::Resolution{
                                            ...
                                            *RESOLUTION.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = ((minx, maxx), (miny, maxy));
                                            allow_err!(stream.send(&Data::Empty).await);
                                        }
                                    }
                                    _ => {
