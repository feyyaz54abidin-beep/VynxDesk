...
pub fn is_service_specified_user(service_id: &str) -> Option<bool> {
    get_service(service_id).map(|s| s.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_specified_user)
}
...
fn get_or_create_service(
    ...
    let mut services = TERMINAL_SERVICES.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).reset_status(is_persistent);
...
fn remove_service(service_id: &str) {
    ...
    if let Some(service) = services.remove(service_id) {
        ...
        let sessions = service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).sessions.clone();
...
pub fn list_services() -> Vec<ServiceMetadata> {
    let services = TERMINAL_SERVICES.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    services
        .iter()
...
fn ensure_cleanup_task() {
    let mut task_handle = CLEANUP_TASK.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn get_terminal_session_count(include_zombie_tasks: bool) -> usize {
    let mut c = TERMINAL_SERVICES.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).len();
    if include_zombie_tasks {
        c += TERMINAL_TASKS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).len();
    }
    c
}
...
fn run(sp: TerminalService, service_id: String) -> ResultType<()> {
    ...
    if let Some(service) = get_service(&service_id) {
        let should_remove = !service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_persistent;
        if should_remove {
...
impl PersistentTerminalService {
    ...
    pub fn list_terminals(&self) -> Vec<(i32, String, u32, Instant)> {
        self.sessions
            .iter()
            .map(|(id, session)| {
                let s = session.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn set_persistent(service_id: &str, is_persistent: bool) -> Result<()> {
    if let Some(service) = get_service(service_id) {
        service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_persistent = is_persistent;
        Ok(())
    } else {
...
impl TerminalServiceProxy {
    pub fn new(
        ...
        let is_persistent =
            is_persistent.unwrap_or(if let Some(service) = get_service(&service_id) {
                service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_persistent
            } else {
                false
            });
    ...
    pub fn handle_action(&mut self, action: &TerminalAction) -> Result<Option<TerminalResponse>> {
        ...
        service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).update_activity();
        match &action.union {
            Some(terminal_action::Union::Open(open)) => {
                self.handle_open(&mut service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()), open)
            }
            ...
            Some(terminal_action::Union::Close(close)) => {
                self.handle_close(&mut service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()), close)
            }
            _ => Ok(None),
        }
    }
    ...
    fn handle_open(
        ...
        if let Some(session_arc) = service.sessions.get(&open.terminal_id) {
            ...
            let mut session = session_arc.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn read_outputs(&self) -> Vec<TerminalResponse> {
        ...
        if !closed_terminals.is_empty() {
            let mut sessions = service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).sessions.clone();
            for terminal_id in closed_terminals {
                let mut exit_code = 0;

                if !self.is_persistent {
                    if let Some(session_arc) = sessions.remove(&terminal_id) {
                        service.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).sessions.remove(&terminal_id);
                        let mut session = session_arc.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
