...
pub mod server {
    ...
    fn run_exit_check() {
        ...
        loop {
            if EXIT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone() {
                break;
            }
            ...
            unsafe {
                ...
                if c.is_none() {
                    let Ok(mut displays) = display_service::try_get_displays() else {
                        ...
                        *EXIT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
                        return;
                    };
    ...
    async fn run_ipc_client(ipc_token: String) {
        ...
        match ipc::connect(1000, postfix).await {
            Ok(mut stream) => {
                ...
                let mut nack = 0;
                loop {
                    if *EXIT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) {
...
pub mod client {
    ...
    fn set_runtime_ipc_token(token: String) {
        *IPC_RUNTIME_TOKEN.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(token);
    }
    ...
    fn clear_runtime_shmem_state() {
        let mut runtime_token = IPC_RUNTIME_TOKEN.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let mut shmem_lock = SHMEM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if let Some(shmem) = shmem_lock.as_mut() {
            clear_ipc_token_in_shmem(shmem);
        }
        *shmem_lock = None;
        let runtime_name = SHMEM_RUNTIME_NAME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take();
        *runtime_token = None;
        drop(runtime_token);
        drop(shmem_lock);
    ...
    fn consume_runtime_ipc_token_if_match(candidate: &str) -> (bool, Option<String>) {
        let mut token = IPC_RUNTIME_TOKEN.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if !token
            .as_deref()
        ...
        let mut shmem_lock = SHMEM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let matched_shmem_name = SHMEM_RUNTIME_NAME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
        *token = None;
        if let Some(shmem) = shmem_lock.as_mut() {
            clear_ipc_token_in_shmem(shmem);
        }
    ...
    fn restore_runtime_ipc_token_after_failed_handshake(
        ...
        let mut shmem_lock = SHMEM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let current_shmem_name = SHMEM_RUNTIME_NAME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
    ...
    fn schedule_starting_timeout_reset(launch_token: u64) {
        std::thread::spawn(move || {
            ...
            let should_reset = {
                ...
                let starting = { *STARTING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) };
                let running = { *RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) };
            ...
            if should_reset {
                ...
                *STARTING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = false;
            }
        });
    }
    ...
    pub(crate) fn start_portable_service(para: StartPara) -> ResultType<()> {
        ...
        let start_result = (|| -> ResultType<()> {
            ...
            *SHMEM_RUNTIME_NAME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(shmem_name);
    ...
    pub fn set_quick_support(v: bool) {
        *QUICK_SUPPORT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = v;
    }
    ...
    impl CapturerPortable {
        ...
        where
            Self: Sized,
        {
            let mut option = SHMEM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    impl TraitCapturer for CapturerPortable {
        fn frame<'a>(&'a mut self, timeout: Duration) -> std::io::Result<Frame<'a>> {
            let mut lock = SHMEM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    async fn start_ipc_server_async(rx: mpsc::UnboundedReceiver<Data>) {
        ...
        let postfix = IPC_SUFFIX;
        let quick_support = QUICK_SUPPORT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
                            ...
                            match result {
                                Ok(stream) => {
                                    ...
                                    tokio::spawn(async move {
                                                ...
                                                res = stream.next() => {
                                                    match res {
                                                        ...
                                                        Ok(Some(Data::DataPortableService(data))) => match data {
                                                            ...
                                                            Pong => {
                                                                nack = 0;
                                                                *RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
                                                                *STARTING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = false;
                                                            },
                                                            ConnCount(None) => {
                                                                if !quick_support {
                                        ...
                                        *RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = false;
                                        *STARTING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = false;
                                    });
                                }
                                Err(err) => {
    ...
    pub fn create_capturer(
        ...
        if portable_service_running != RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone() {
    ...
    pub fn get_cursor_info(pci: PCURSORINFO) -> BOOL {
        if RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone() {
            let mut option = SHMEM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            option
                .as_mut()
    ...
    pub fn running() -> bool {
        RUNNING.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
    }
}
