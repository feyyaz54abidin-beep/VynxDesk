...
fn display_info_of(display: i32) -> Option<DrmDisplayInfo> {
    match &*DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) {
...
impl IpcDrmCapturer {
    ...
    fn note_session_without_frame(&self) {
        ...
        let mut map = DRM_DISPLAY_HEALTH.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
impl TraitCapturer for IpcDrmCapturer {
    fn frame<'a>(&'a mut self, timeout: Duration) -> io::Result<Frame<'a>> {
        ...
        {
            let mut slot = self.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            loop {
            ...
            if let Some((w, h, fmt, buf)) = slot.latest.take() {
                ...
                if scrap::wayland::display::wayland_snapshot_generation() != self.snapshot_gen {
                    self.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).recycle(buf);
                ...
                if self.transform == 0 {
                    ...
                    self.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).recycle(previous);
                ...
                if !self.got_frame {
                    ...
                    if let Some(key) = &self.connector {
                        if let Some(h) = DRM_DISPLAY_HEALTH.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_mut(key) {
            ...
            Data::DrmFrameDmabuf(desc) => {
                ...
                match conv.convert(&mut ddesc, received_fd) {
                    Ok((data, w, h, fmt)) => {
                        ...
                        let mut buf = shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take_free().unwrap_or_default();
                        buf.clear();
                        buf.extend_from_slice(data);
                        let mut slot = shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn set_drm_cursor(display: i32, epoch: u64, c: DrmCursorData) {
    let mut map = DRM_CURSOR.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn with_drm_cursor<T>(f: impl Fn(&DrmCursorData) -> T) -> Option<T> {
    let map = DRM_CURSOR.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    map.values()
        .map(|(_, c)| c)
...
pub(crate) fn is_available_cached() -> bool {
    matches!(&*DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()), ProbeState::Available(..))
}
...
fn availability() -> Availability {
    let (verdict, stale_no) = {
        let st = DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    if DRM_PROBE_IN_FLIGHT.swap(true, Ordering::AcqRel) {
        ...
        return match &*DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) {
...
fn probe_and_publish() -> Availability {
    ...
    let mut st = DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub(super) fn warm_availability() {
    ...
    for _ in 0..10 {
        ...
        if matches!(&*DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()), ProbeState::Available(..)) {
            return;
        }
        match query_displays() {
            Ok(list) if !list.is_empty() => {
                log::info!("drm: consumer cache warmed ({} displays) at startup", list.len());
                publish_probe_state(&mut DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()), ProbeState::Available(Instant::now(), list));
                return;
            }
...
pub(super) fn display_count_and_any_demoted() -> Option<(usize, bool)> {
    ...
    let (len, keys): (usize, Vec<String>) = match &*DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) {
    ...
    let any_demoted = if len > 1 {
        let health = DRM_DISPLAY_HEALTH.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        keys.iter()
...
pub(super) fn get_display_infos_and_primary() -> Option<(Vec<DisplayInfo>, usize)> {
    let list = match &*DRM_STATE.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) {
...
mod drm_capturer_tests {
    ...
    fn a_lone_display_goes_offline_only_when_its_fallback_was_rejected() {
        ...
        DRM_DISPLAY_HEALTH.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).insert(key.clone(), demoted);
    ...
    fn put_frame(c: &IpcDrmCapturer, w: usize, h: usize) {
        let mut buf = c.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take_free().unwrap_or_default();
        buf.clear();
        buf.resize(w * h * 4, 0);
        let mut slot = c.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn a_delivered_frame_clears_the_streak_but_keeps_the_cadence_and_the_convert_verdict() {
        ...
        let h = {
            let map = DRM_DISPLAY_HEALTH.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn frame_buffers_circulate_instead_of_being_reallocated() {
        ...
        assert!(matches!(c.frame(Duration::from_millis(50)), Ok(_)));
        assert!(
            c.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).free.iter().any(|b| b.is_some()),
    ...
    fn two_idle_buffers_are_both_kept_rather_than_one_being_dropped() {
        ...
        put_frame(&c, 64, 32);
        assert!(matches!(c.frame(Duration::from_millis(50)), Ok(_)));
        while c.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).take_free().is_some() {}

        put_frame(&c, 64, 32); // fills a fresh buffer (nothing on offer) and publishes it
        put_frame(&c, 64, 32); // supersedes it -> deposit #1
        assert_eq!(
            c.shared.slot.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).free.iter().flatten().count(),
            1,
