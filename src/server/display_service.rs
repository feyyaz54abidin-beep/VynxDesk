...
pub(super) fn set_wayland_uinput_rect(rect: (i32, i32, i32, i32)) {
    WAYLAND_UINPUT_RECT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).rect = Some(rect);
}
...
pub(super) fn wayland_uinput_rect() -> Option<(i32, i32, i32, i32)> {
    WAYLAND_UINPUT_RECT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).rect
}
...
pub(super) fn set_wayland_layout_baseline(baseline: Vec<scrap::wayland::display::DisplayRect>) {
    ...
    WAYLAND_LAYOUT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).reset_baseline(baseline);
}
...
pub(super) fn remap_wayland_uinput_coord(x: i32, y: i32) -> (i32, i32) {
    ...
    let lock = WAYLAND_LAYOUT.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
fn refresh_wayland_uinput_rect_if_changed() {
    ...
    {
        let mut lock = WAYLAND_UINPUT_RECT.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    let (live_changed, mut drifted) = {
        let mut layout = WAYLAND_LAYOUT.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        #[cfg(feature = "drm")]
    ...
    #[cfg(feature = "drm")]
    {
        ...
        if PROMOTION_OWED.load(Ordering::Acquire) && super::drm_capturer::is_available_cached() {
            ...
            WAYLAND_LAYOUT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).live = live_rects.clone();
            drifted = false;
        }
    }
    #[cfg(not(feature = "drm"))]
    ...
    let mut range_ok = WAYLAND_UINPUT_RECT.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).rect == Some(rect);
    if !range_ok {
...
pub fn temp_ignore_displays_changed() -> SimpleCallOnReturn {
    ...
    SimpleCallOnReturn {
        ...
        f: Box::new(move || {
            ...
            SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).is_synced = false;
        }),
    }
}
...
pub(super) fn check_display_changed(
    ...
    let lock = SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    #[cfg(target_os = "linux")]
    {
        if !is_x11() {
            ...
            if super::drm_capturer::is_available_cached() {
                let synced = !SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).displays.is_empty();
                ...
                if !synced || !scrap::wayland::display::wayland_lookup_suppressed() {
                    if let Some(displays) = super::drm_capturer::get_display_infos() {
                        ...
                        {
                            SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).check_changed(&displays);
                        }
                    }
                }
            }
...
fn get_displays_msg() -> Option<Message> {
    let displays = SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get_update_sync_displays()?;
...
pub(super) fn get_sync_displays() -> Vec<DisplayInfo> {
    SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).displays.clone()
}

pub(super) fn get_display_info(idx: usize) -> Option<DisplayInfo> {
    SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).displays.get(idx).cloned()
}
...
pub fn has_non_drm_backed_display() -> bool {
    match super::drm_capturer::display_count_and_any_demoted() {
        ...
        Some((count, any_demoted)) => {
            count < SYNC_DISPLAYS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).displays.len() || any_demoted
        }
        None => false,
    }
}
