...
impl Connection {
                    ...
                    if let Some(res) = res {
                        match res {
                            ...
                            Ok(bytes) => {
                                ...
                                conn.session_last_recv_time.as_mut().map(|t| *t.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Instant::now());
                ...
                _ = test_delay_timer.tick() => {
                    ...
                    if conn.last_test_delay.is_none() && !(conn.port_forward_socket.is_some() && conn.authorized) {
                        ...
                        msg_out.set_test_delay(TestDelay{
                            ...
                            target_bitrate: video_service::VIDEO_QOS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).bitrate(),
                            ..Default::default()
                        });
                    ...
                    if conn.is_authed_remote_conn() || conn.view_camera {
                        if let Some(last_test_delay) = conn.last_test_delay {
                            video_service::VIDEO_QOS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).user_delay_response_elapsed(id, last_test_delay.elapsed().as_millis());
                        }
                    }
                }
    ...
    fn handle_input(receiver: std_mpsc::Receiver<MessageInput>, tx: Sender) {
        ...
        #[cfg(all(target_os = "windows", feature = "vynx-input-bridge"))]
        enigo::bridge_session_started();
        loop {
    ...
    fn decay_id_whitelist_failures(&self) {
        decay_stale_failures(
            &mut LOGIN_FAILURES[FAILURE_IDX_ID_WHITELIST]
                .lock()
    ...
    fn on_remote_authorized(&self) {
        ...
        if config::option2bool(
            ...
            let mut wallpaper = WALLPAPER_REMOVER
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    fn check_update_temporary_password(&self, temporary_password_success: bool) {
        ...
        let mut state = TEMPORARY_PASSWORD_FAILURES
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    async fn on_message(&mut self, msg: Message) -> bool {
        ...
        if let Some(message::Union::LoginRequest(lr)) = msg.union {
            ...
            if let Some(lr) = _s.lr.clone().take() {
                ...
                let uuid_old = SWITCH_SIDES_UUID
                    .lock()
                    .unwrap_or_else(|poisoned| poisoned.into_inner())
                    .remove(&lr.my_id);
    ...
    fn portable_check(&mut self) {
        ...
        if self.authorized {
            ...
            let uac = crate::video_service::IS_UAC_RUNNING
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner())
                .clone();
            if p.last_uac != uac {
...
pub fn insert_pending_switch_sides_uuid(id: String, uuid: uuid::Uuid) -> bool {
    let mut uuids = PENDING_SWITCH_SIDES_UUID
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
        ...
        loop {
            match rx.recv() {
                Ok((conn_count, remote_count)) => {
                    ...
                    *WAKELOCK_KEEP_AWAKE_OPTION
                        .lock()
                        .unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(keep_awake);
...
pub fn get_control_permission_state(
    ...
    let control_permissions = CONTROL_PERMISSIONS_ARRAY
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
...
mod raii {
    ...
    impl Drop for ConnectionID {
        fn drop(&mut self) {
            let mut active_conns_lock = ALIVE_CONNS
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    impl AuthedConnID {
        ...
        pub fn check_wake_lock_on_setting_changed() {
            ...
            let cached = *WAKELOCK_KEEP_AWAKE_OPTION
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    impl Drop for ControlPermissionsID {
        fn drop(&mut self) {
            if self.control_permissions.is_some() {
                let mut lock = CONTROL_PERMISSIONS_ARRAY
                    .lock()
                    .unwrap_or_else(|poisoned| poisoned.into_inner());
