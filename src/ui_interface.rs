...
pub fn refresh_options() {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        *OPTIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Config::get_options();
    }
}
...
pub fn get_option<T: AsRef<str>>(key: T) -> String {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        let map = OPTIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn get_options() -> String {
    let options = {
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
        {
            OPTIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner())
        }
        #[cfg(any(target_os = "android", target_os = "ios"))]
        {
            Config::get_options()
...
pub fn get_sound_inputs() -> Vec<String> {
    ...
    #[cfg(not(target_os = "linux"))]
    {
        ...
        std::thread::spawn(move || *cloned.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = get_sound_inputs_())
            .join()
            .ok();
        for name in inputs.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).drain(..) {
            a.push(name);
        }
    }
    #[cfg(target_os = "linux")]
...
pub fn set_options(m: HashMap<String, String>) {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        *OPTIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = m.clone();
...
pub fn set_option(key: String, value: String) {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        let mut options = OPTIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        if value.is_empty() {
            options.remove(&key);
        } else {
...
pub fn get_mouse_time() -> f64 {
    UI_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).mouse_time as f64
}
...
pub fn get_connect_status() -> UiStatus {
    UI_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
}
...
pub fn temporary_password() -> String {
    ...
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    return TEMPORARY_PASSWD.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
}
...
pub fn reset_async_job_status() {
    *ASYNC_JOB_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = get_init_async_job_status();
}
...
pub fn http_request(url: String, method: String, body: Option<String>, header: String) {
    ...
    std::thread::spawn(move || {
        ...
        current_request.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).insert(url, res);
    });
}

#[inline]
pub fn get_async_http_status(url: String) -> Option<String> {
    match ASYNC_HTTP_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).get(&url) {
        None => None,
...
pub fn post_request(url: String, body: String, header: String) {
    *ASYNC_JOB_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = " ".to_owned();
    std::thread::spawn(move || {
        *ASYNC_JOB_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = match crate::post_request_sync(url, body, &header) {
...
pub fn get_async_job_status() -> String {
    ASYNC_JOB_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
}
...
pub fn check_zombie() {
    ...
    loop {
        let mut lock = CHILDREN.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let mut n = 0;
...
pub fn recent_sessions_updated() -> bool {
    let mut children = CHILDREN.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    if children.0 {
        children.0 = false;
        true
    } else {
...
pub fn start_option_status_sync() {
    let _sender = SENDER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
}
                    ...
                    res = c.next() => {
                        match res {
                            ...
                            Ok(Some(ipc::Data::MouseMoveTime(v))) => {
                                mouse_time = v;
                                UI_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).mouse_time = v;
                            }
                            Ok(Some(ipc::Data::Options(Some(v)))) => {
                                *OPTIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = v;
                                *OPTION_SYNCED.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = true;
                            }
                            Ok(Some(ipc::Data::Config((name, Some(value))))) => {
                                if name == "id" {
                                    ...
                                    *TEMPORARY_PASSWD.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = value;
                                }
                            }
                            #[cfg(feature = "flutter")]
                            ...
                            Ok(Some(ipc::Data::OnlineStatus(Some((mut x, _c))))) => {
                                ...
                                *UI_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = UiStatus {
                                    status_num: x as _,
                                    #[cfg(not(feature = "flutter"))]
                                    key_confirmed: _c,
                                    #[cfg(not(any(target_os = "android", target_os = "ios")))]
                            ...
                            Ok(Some(ipc::Data::ControlPermissionsRemoteModify(v))) => {
                                *IS_REMOTE_MODIFY_ENABLED_BY_CONTROL_PERMISSIONS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = v;
                            }
                            #[cfg(target_os = "windows")]
                            Ok(Some(ipc::Data::FileTransferEnabledState(v))) => {
                                if let Some(enabled) = v {
                                    let mut lock = IS_FILE_TRANSFER_ENABLED.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
                                    if *lock != v {
...
pub fn option_synced() -> bool {
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
        OPTION_SYNCED.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
    }
    #[cfg(any(target_os = "android", target_os = "ios"))]
    {
        true
...
pub async fn change_id_shared(id: String, old_id: String) -> String {
    ...
    *ASYNC_JOB_STATUS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = res.clone();
    res
}
...
pub async fn change_id_shared_(id: String, old_id: String) -> &'static str {
    ...
    for rendezvous_server in rendezvous_servers {
        ...
        futs.push(tokio::spawn(async move {
            ...
            if !tmp.is_empty() {
                *err.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = tmp;
            }
        }));
    }
    join_all(futs).await;
    let err = *err.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    if err.is_empty() {
        #[cfg(not(any(target_os = "android", target_os = "ios")))]
