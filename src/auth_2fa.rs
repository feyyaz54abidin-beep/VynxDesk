...
pub fn generate2fa() -> String {
    ...
    if let Ok(info) = TOTPInfo::gen_totp_info(id, 6) {
        if let Ok(totp) = info.new_totp() {
            ...
            *CURRENT_2FA.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some((info, totp));
            return code;
        }
    }
    "".to_owned()
...
pub fn verify2fa(code: String) -> bool {
    if let Some((info, totp)) = CURRENT_2FA.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_ref() {
