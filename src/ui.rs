...
pub fn start(args: &mut [String]) {
    ...
    #[cfg(windows)]
    if args.is_empty() {
        crate::platform::windows::set_vynx_caption_colors(frame.get_hwnd() as _);
    }
    #[cfg(target_os = "macos")]
    ...
    if args.is_empty() {
        ...
        page = "cm.html";
        *cm::HIDE_CM.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = crate::ipc::get_config("hide_cm")
            .ok()
            .flatten()
            .unwrap_or_default()
            == "true";
        ...
        && args.len() > 1
    {
        ...
        frame.register_behavior("native-remote", move || {
            ...
            #[cfg(not(feature = "flutter"))]
            {
                *CUR_SESSION.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(handler.inner());
            }
            Box::new(handler)
        });
        page = "remote.html";
    ...
    #[cfg(not(feature = "inline"))]
    {
        ...
        if !path.exists() {
            if let Ok(exe) = std::env::current_exe() {
                if let Some(parent) = exe.parent() {
                    let candidate = parent.join("src").join("ui").join(page);
                    if candidate.exists() {
                        path = candidate;
                    } else if let Some(gp) = parent.parent() {
                        let candidate2 = gp.join("src").join("ui").join(page);
        ...
        if !path.exists() {
            let hardcoded = std::path::PathBuf::from(r"C:\Users\Lionis\Rustdesksrc\src\ui").join(page);
            if hardcoded.exists() {
                path = hardcoded;
            }
        }
        frame.load_file(&format!("file://{}", path.to_string_lossy().replace('\\', "/")));
    }
    let hide_cm = *cm::HIDE_CM.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
impl UI {
    ...
    fn get_software_update_url(&self) -> String {
        crate::SOFTWARE_UPDATE_URL.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone()
    }
...
pub fn value_crash_workaround(values: &[Value]) -> Arc<Vec<Value>> {
    ...
    STUPID_VALUES.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).push(persist.clone());
    persist
}
