...
pub fn main_device_id(id: String) {
    *crate::common::DEVICE_ID.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = id;
}

pub fn main_device_name(name: String) {
    *crate::common::DEVICE_NAME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = name;
}
...
pub fn session_get_last_audit_note(session_id: SessionID) -> SyncReturn<String> {
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        SyncReturn(session.last_audit_note.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone())
    } else {
        SyncReturn("".to_owned())
    }
}
...
pub fn session_set_audit_guid(session_id: SessionID, guid: String) {
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        *session.audit_guid.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = guid;
    }
}

pub fn session_get_audit_guid(session_id: SessionID) -> SyncReturn<String> {
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        SyncReturn(session.audit_guid.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone())
    } else {
        SyncReturn("".to_owned())
    }
}
