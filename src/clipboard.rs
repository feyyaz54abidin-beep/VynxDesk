...
pub fn check_clipboard(
    ...
    *LAST_MULTI_CLIPBOARDS.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = clipboards;
    Some(msg)
}
...
pub fn try_empty_clipboard_files_sync(_side: ClipboardSide, _conn_id: i32) -> ResultType<()> {
    let mut ctx = CLIPBOARD_CTX.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    if ctx.is_none() {
...
impl ClipboardContext {
    ...
    fn get_formats_filter(
        ...
        let _lock = ARBOARD_MTX.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub fn get_current_clipboard_msg(
    ...
    let mut multi_clipboards = LAST_MULTI_CLIPBOARDS.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub mod clipboard_listener {
    ...
    impl ClipboardHandler for Handler {
        fn on_clipboard_change(&mut self) -> CallbackResult {
            let sub_lock = self.subscribers.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn subscribe(name: String, tx: Sender<CallbackResult>) -> ResultType<()> {
        ...
        let mut listener_lock = CLIPBOARD_LISTENER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        listener_lock
            .subscribers
            .lock()
            .unwrap()
    ...
    pub fn unsubscribe(name: &str) {
        ...
        let mut listener_lock = CLIPBOARD_LISTENER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
        let is_empty = {
            let mut sub_lock = listener_lock.subscribers.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
