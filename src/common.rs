...
pub fn username() -> String {
    ...
    #[cfg(any(target_os = "android", target_os = "ios"))]
    return DEVICE_NAME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).clone();
}
...
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    ...
    if get_version_number(&latest_release_version) > get_version_number(crate::VERSION) {
        ...
        *SOFTWARE_UPDATE_URL.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = response_url;
    } else {
        *SOFTWARE_UPDATE_URL.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = "".to_string();
    }
    Ok(())
}
...
pub async fn test_ipv6() -> Option<tokio::task::JoinHandle<()>> {
    ...
    PUBLIC_IPV6_ADDR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).1 = Some(Instant::now());
    ...
    match test_bind_ipv6().await {
        Ok(mut addr) => {
            if let std::net::IpAddr::V6(ip) = addr.ip() {
                ...
                {
                    addr.set_port(0);
                    PUBLIC_IPV6_ADDR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).0 = Some(addr);
            ...
            for iface in ifaces {
                if let if_addrs::IfAddr::V6(v6) = iface.addr {
                        ...
                        && (ip.segments()[0] & 0xe000) == 0x2000
                    {
                        ...
                        *PUBLIC_IPV6_ADDR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) =
...
pub async fn get_ipv6_socket() -> Option<(Arc<UdpSocket>, bytes::Bytes)> {
    let Some(addr) = PUBLIC_IPV6_ADDR.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).0 else {
        return None;
    };
