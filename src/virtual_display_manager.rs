...
pub mod rustdesk_idd {
    ...
    pub fn plug_in_headless() -> ResultType<()> {
        let mut manager = VIRTUAL_DISPLAY_MANAGER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn is_virtual_display(name: &str) -> bool {
        let lock = VIRTUAL_DISPLAY_MANAGER.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
...
pub mod amyuni_idd {
    ...
    fn check_install_driver(is_async: &mut bool) -> ResultType<()> {
        let _l = LOCK.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
    ...
    pub fn reset_all() -> ResultType<()> {
        ...
        *LAST_PLUG_IN_HEADLESS_TIME.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = None;
        Ok(())
    }
    ...
    pub fn plug_in_headless() -> ResultType<()> {
        let mut tm = LAST_PLUG_IN_HEADLESS_TIME.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
