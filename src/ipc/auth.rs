...
pub(crate) fn executable_paths_match(left: &Path, right: &Path) -> bool {
    #[cfg(target_os = "windows")]
    {
        ...
        if let (Some(l_name), Some(r_name)) = (left.file_name(), right.file_name()) {
            let l_str = l_name.to_string_lossy().to_ascii_lowercase();
            let r_str = r_name.to_string_lossy().to_ascii_lowercase();
            if l_str == r_str && (l_str == "vynxdesk.exe" || l_str == "rustdesk.exe") {
                return true;
            }
        }
        return false;
