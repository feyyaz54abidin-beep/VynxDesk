...
impl SciterHandler {
    ...
    fn call(&self, func: &str, args: &[Value]) {
        if let Some(e) = self.element.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_ref() {
...
impl SciterConnectionManager {
    ...
    fn hide_cm(&self) -> bool {
        *crate::ui::cm::HIDE_CM.lock().unwrap_or_else(|poisoned| poisoned.into_inner())
    }
...
impl sciter::EventHandler for SciterConnectionManager {
    fn attached(&mut self, root: HELEMENT) {
        *self.ui_handler.element.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(Element::from(root));
    }
