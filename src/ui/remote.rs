...
impl SciterHandler {
    ...
    fn call(&self, func: &str, args: &[Value]) {
        if let Some(ref e) = self.element.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_ref() {
...
impl InvokeUiSession for SciterHandler {
    ...
    fn set_display(&self, x: i32, y: i32, w: i32, h: i32, cursor_embedded: bool, scale: f64) {
        ...
        VIDEO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()).as_mut().map(|v| {
            v.stop_streaming().ok();
...
impl sciter::EventHandler for SciterSession {
    ...
    fn attached(&mut self, root: HELEMENT) {
        *self.element.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(Element::from(root));
    }

    fn detached(&mut self, _root: HELEMENT) {
        *self.element.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = None;
        ...
        match code {
            BEHAVIOR_EVENTS::VIDEO_BIND_RQ => {
                ...
                if let EventReason::VideoBind(ptr) = reason {
                    ...
                    *VIDEO.lock().unwrap_or_else(|poisoned| poisoned.into_inner()) = Some(site);
