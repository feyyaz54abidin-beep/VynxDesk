...
index = open('src/ui/index.html').read() \
    ...
    .replace('@import url(launcher.css);', open('src/ui/launcher.css').read()) \
...
for asset in ('logo.webp', 'terrain-middle.webp', 'planet.webp'):
    path = 'res/vynx-theme/' + asset
    with open(path, 'rb') as image:
        embedded = 'data:image/webp;base64,' + base64.b64encode(image.read()).decode('ascii')
    index = index.replace('../../' + path, embedded)
